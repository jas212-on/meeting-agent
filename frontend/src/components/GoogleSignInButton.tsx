import React, { useEffect, useRef, useState } from "react";

interface GoogleSignInButtonProps {
  onSuccess: (credential: string) => void;
  onError?: (error: string) => void;
  text?: "signin_with" | "signup_with" | "continue_with" | "signin";
  disabled?: boolean;
}

export const GoogleSignInButton: React.FC<GoogleSignInButtonProps> = ({
  onSuccess,
  onError,
  text = "continue_with",
  disabled = false,
}) => {
  const containerRef = useRef<HTMLDivElement>(null);
  const [isGsiLoaded, setIsGsiLoaded] = useState(false);
  const onSuccessRef = useRef(onSuccess);
  onSuccessRef.current = onSuccess;
  const onErrorRef = useRef(onError);
  onErrorRef.current = onError;

  const clientId =
    import.meta.env.VITE_GOOGLE_CLIENT_ID ||
    "1023952748673-eevrl0u7q5jrujnm7efe8kgtcaiuvlrf.apps.googleusercontent.com";

  useEffect(() => {
    let intervalId: any = null;
    let attempts = 0;

    const setupGoogleButton = () => {
      if (window.google?.accounts?.id && containerRef.current) {
        try {
          window.google.accounts.id.initialize({
            client_id: clientId,
            callback: (res: { credential: string }) => {
              if (res.credential) {
                onSuccessRef.current(res.credential);
              } else if (onErrorRef.current) {
                onErrorRef.current("Failed to obtain Google credentials.");
              }
            },
          });

          containerRef.current.innerHTML = "";

          window.google.accounts.id.renderButton(containerRef.current, {
            theme: "outline",
            size: "large",
            text: text === "signup_with" ? "signup_with" : "continue_with",
            shape: "rectangular",
            width: 320,
            logo_alignment: "left",
          });
          setIsGsiLoaded(true);
          return true;
        } catch (err) {
          console.error("Failed to render Google Sign-In button:", err);
        }
      }
      return false;
    };

    if (!setupGoogleButton()) {
      intervalId = setInterval(() => {
        attempts++;
        if (setupGoogleButton() || attempts >= 40) {
          clearInterval(intervalId);
        }
      }, 100);
    }

    return () => {
      if (intervalId) clearInterval(intervalId);
    };
  }, [clientId, text]);

  const handleManualClick = () => {
    if (disabled) return;
    if (window.google?.accounts?.id?.prompt) {
      window.google.accounts.id.prompt();
    } else if (onErrorRef.current) {
      onErrorRef.current("Google Identity Services is still loading. Please check your connection.");
    }
  };

  return (
    <div className="google-auth-wrapper">
      <div
        ref={containerRef}
        className={`gsi-container ${disabled ? "pointer-events-none opacity-60" : ""}`}
        style={{ display: isGsiLoaded ? "flex" : "none", justifyContent: "center", width: "100%" }}
      />
      {!isGsiLoaded && (
        <button
          type="button"
          className="google-auth-fallback-btn"
          onClick={handleManualClick}
          disabled={disabled}
        >
          <svg className="w-4 h-4 flex-shrink-0" viewBox="0 0 24 24">
            <path
              fill="#4285F4"
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
            />
            <path
              fill="#34A853"
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
            />
            <path
              fill="#FBBC05"
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z"
            />
            <path
              fill="#EA4335"
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z"
            />
          </svg>
          <span>{text === "signup_with" ? "Sign up with Google" : "Continue with Google"}</span>
        </button>
      )}
    </div>
  );
};
